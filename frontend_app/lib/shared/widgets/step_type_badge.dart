...
class StepTypeBadge extends StatelessWidget {
  ...
  factory StepTypeBadge.fromStepType(String stepType) {
    return switch (stepType) {
      'new_word_intro' => const StepTypeBadge(
        label: 'Новое слово',
        variant: BadgeVariant.newWord,
      ),
      'meaning_choice' => const StepTypeBadge(
        label: 'AR → EN',
        variant: BadgeVariant.accent,
      ),
      'review_card' => const StepTypeBadge(
        label: 'Повторение',
        variant: BadgeVariant.neutral,
      ),
      'reinforcement' => const StepTypeBadge(
        label: 'Закрепление',
        variant: BadgeVariant.neutral,
      ),
      'audio_to_meaning' => const StepTypeBadge(
        label: 'Аудирование',
        variant: BadgeVariant.listening,
      ),
      'translation_to_word' => const StepTypeBadge(
        label: 'EN → AR',
        variant: BadgeVariant.accent,
      ),
      String s when s.startsWith('ayah_build') => const StepTypeBadge(
        label: 'Ayah Build',
        variant: BadgeVariant.accent,
      ),
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Text(label, style: AppTypography.labelSm.copyWith(color: text)),
    );
  }
}
